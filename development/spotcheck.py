...
for name, df in datasets.items():
    ...
    print(f"\n[{name}] Non-standard missing values (in string columns):")
    obj_cols = df.select_dtypes(include=['object', 'string']).columns
    for col in obj_cols:
        empty_str_count = (df[col] == "").sum()
        str_nan_count = (df[col].astype(str).str.lower() == "nan").sum()
        str_null_count = (df[col].astype(str).str.lower() == "null").sum()
        none_str_count = (df[col].astype(str).str.lower() == "none").sum()
...
for name, df in datasets.items():
    ...
    print(f"\n[{name}] Fully duplicated rows: {dup_rows}")
    if dup_rows > 0:
        print("Sample of duplicated rows:")
        print(df[df.duplicated(keep=False)].sort_values(by=df.columns.tolist()[:2]).head())
...
for name, df in datasets.items():
    ...
    if 'Title' in df.columns:
        title_col = 'Title'
    elif 'Title_gsheets' in df.columns:
        title_col = 'Title_gsheets'

    if title_col:
        title_dupes = df.duplicated(subset=[title_col]).sum()
        print(f"[{name}] Duplicated titles in '{title_col}': {title_dupes}")
        if title_dupes > 0:
            print(f"Sample of duplicated titles in {name}:")
...
for name, df in datasets.items():
    obj_cols = df.select_dtypes(include=['object', 'string']).columns
    found_hidden = False
    print(f"[{name}] Checking string columns for hidden characters...")
    for col in obj_cols:
        # Check leading/trailing spaces
        leading_trailing = df[col].astype(str).str.contains(r'^\s+|\s+$', regex=True, na=False).sum()
        # Check internal newlines
        newlines = df[col].astype(str).str.contains(r'\n', regex=True, na=False).sum()
        # Check non-breaking spaces (\xa0)
        nbsp = df[col].astype(str).str.contains(r'\xa0', regex=True, na=False).sum()

        if any([leading_trailing, newlines, nbsp]):
             print(f"  {col}: Leading/Trailing Spaces={leading_trailing}, Newlines={newlines}, NBSP={nbsp}")
             found_hidden = True
    if not found_hidden:

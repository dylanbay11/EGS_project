...
def _():
    ...
    def load_and_clean_data(filepath):
        ...
        df = pd.read_csv(filepath, skiprows=[1]) # I strongly suggest removing skiprows=[1] as your new CSV format no longer contains a totals row.
    
        # Clean 'GG' column
        df['is_good_game'] = df['good_game'].apply(lambda x: True if str(x).strip() == '1' else False)
    
        # Clean 'Start' column to datetime
        df['Start Date'] = pd.to_datetime(df['start_date'], errors='coerce')
    ...
    def visualize_duplicates(df, output_dir):
        ...
        games_clean = df.dropna(subset=['title'])
    
        duplicate_counts = games_clean['title'].value_counts().reset_index()
